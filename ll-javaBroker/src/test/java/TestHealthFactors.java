    ...
    try {
      ...
      if (vistaSignonResult.getSignonSucceeded()) {
        ...
        */
        ...
        PatientEncounterRpc patientEncounterRpc = 
            new PatientEncounterRpc(rpcBroker);
        List<String> pceList = new ArrayList<String>();
        pceList.add("HDR^0^^3140701.11015;E");
        pceList.add("VST^DT^3140701.11015");
        pceList.add("VST^PT^10190057");
        pceList.add("HF+^606407^^LOM AVS Printed^@^^^^^1^");
        String noteIen = "29520952";
        String locationIen = "2872";
        String x = patientEncounterRpc.savePceData(pceList, noteIen, locationIen);
        System.out.println(x);

...
public class FusionChartsExportHelper {
    ...
    public static String getExportHandlerClassName(String strFormat) {
        ...
        String path = ExportConfiguration.RESOURCEPACKAGE + "." + ExportConfiguration.EXPORTHANDLER + exporterSuffix.toUpperCase();
        return path;
    }
    ...
    public static String getUniqueFileName(String filePath, String extension) {
        ...
        (new StringBuilder(String.valueOf(filePath))).append(".").append(extension).toString();

        String uniqueFileName;
        do {
            if(!ExportConfiguration.FILESUFFIXFORMAT.equalsIgnoreCase("TIMESTAMP")) {
                uniqueFileName = filePath + uid + "_" + Math.random();
            } else {
                SimpleDateFormat sdf = new SimpleDateFormat("dMyHms");
                String date = sdf.format(Calendar.getInstance().getTime());
                uniqueFileName = filePath + uid + "_" + date + "_" + Calendar.getInstance().getTimeInMillis();
            }

...
public class PDFGenerator {
    ...
    public PDFGenerator(String data, ChartMetadata metadata) {
        this.setBitmapData(data, metadata);
    }
    ...
    private byte[] addImageToPDF(int id, boolean isCompressed) {
        ...
        try {
            ...
            this.logger.severe("Exception while parsing image data for PDF: " + var12.toString());
        }
    ...
    private byte[] getBitmapData24(int id) {
        ...
        while(rows.hasMoreElements()) {
            ...
            while(pixels.hasMoreElements()) {
                ...
                color = pixelData[0];
                int repeat1 = Integer.parseInt(pixelData[1]);
                ...
                byte[] e = this.hexToBytes((String)color);
                byte[] repeatedBytes = this.repeatBytes(e, repeat1);

                try {
                    imageData24OS.write(repeatedBytes);
                    imageData24OS.flush();
                } catch (IOException var14) {
                    this.logger.severe("Exception while writing image data for PDF: " + var14.toString());
                }
            }
        }
        ...
        try {
            ...
            this.logger.severe("Exception while closing stream for PDF: " + var13.toString());
        }
    ...
    public byte[] getPDFObjects(boolean isCompressed) {
        ...
        try {
            ...
            xRefList.add(this.calculateXPos("" + PDFBytesOS.size()));
            PDFBytesOS.write(strTmpObj.getBytes());
            ...
            for(int e = 0; e < this.pageIndex; ++e) {
                strTmpObj = strTmpObj + ((e + 1) * 3 + 1) + " 0 R\n";
            }

            strTmpObj = strTmpObj + "] /Count " + this.pageIndex + " >>\nendobj\n";
            xRefList.add(this.calculateXPos("" + PDFBytesOS.size()));
            PDFBytesOS.write(strTmpObj.getBytes());
            ChartMetadata var14 = null;
            this.logger.info("Gathering data for  each page");

            for(int xRef0 = 0; xRef0 < this.pageIndex; ++xRef0) {
                var14 = ((ExportBean)this.pagesData.get(xRef0)).getMetadata();
                int iWidth = var14.getWidth();
                int iHeight = var14.getHeight();
                strTmpObj = (xRef0 + 2) * 3 - 2 + " 0 obj\n<<\n/Type /Page /Parent 3 0 R \n/MediaBox [ 0 0 " + iWidth + " " + iHeight + " ]\n/Resources <<\n/ProcSet [ /PDF ]\n/XObject <</R" + (xRef0 + 1) + " " + (xRef0 + 2) * 3 + " 0 R>>\n>>\n/Contents [ " + ((xRef0 + 2) * 3 - 1) + " 0 R ]\n>>\nendobj\n";
                xRefList.add(this.calculateXPos("" + PDFBytesOS.size()));
                PDFBytesOS.write(strTmpObj.getBytes());
                xRefList.add(this.calculateXPos("" + PDFBytesOS.size()));
                PDFBytesOS.write(this.getXObjResource(xRef0).getBytes());
                byte[] imgPDFBytes = this.addImageToPDF(xRef0, isCompressed);
                xRefList.add(this.calculateXPos("" + PDFBytesOS.size()));
                PDFBytesOS.write(imgPDFBytes);
            }

            String var15 = (String)xRefList.get(0) + (xRefList.size() - 1) + "\n";
            xRefList.set(0, var15);
            String trailer = this.getTrailer(PDFBytesOS.size(), xRefList.size() - 1);
            PDFBytesOS.write(this.arrayToString(xRefList, "").getBytes());
            ...
            this.logger.severe("Exception while writing PDF data: " + var13.toString());
        }
    ...
    private byte[] hexToBytes(char[] hex) {
        ...
        for(int i = 0; i < length; ++i) {
            ...
            raw[i] = (byte)((value & 255) <= 127?value:value - 256);
        }
    ...
    private byte[] hexToBytes(String hex) {
        return this.hexToBytes((char[])hex.toCharArray());
    }

...
import gov.va.med.lom.javaUtils.misc.DBConnectionPool;
...
public class SheetServiceImpl extends BaseServiceImpl implements BaseService, SheetService {
  ...
  private static final long EXPIRED_REMOTE_MEDS = 30 * 24 * 60 * 60 * 1000; // 30 days
  ...
  @EJB
  private MedDescriptionsDao medDescriptionsDao;
  
  @EJB
  private EncounterCacheDao encounterCacheDao;
  
  @EJB
  private EncounterDao encounterDao;
  
  @EJB
  private EncounterProviderDao encounterProviderDao;
  ...
  public ServiceResponse<BasicDemographics> getBasicDemographics(ISecurityContext securityContext, EncounterInfo encounterInfo) {
    ...
    BasicDemographics basicDemographics = (BasicDemographics)MEM_CACHE.get(encounterInfo.getFacilityNo(), 
        encounterInfo.getPatientDfn(), "demographics");
    ...
    if (patientDob != null) {
      basicDemographics.setDob(!AvsProps.IS_DEMO ? FMDateUtils.dateToFMDate(patientDob) : AvsProps.DEMO_PT_DOB_DM);
    }
    basicDemographics.setDobStr(!AvsProps.IS_DEMO ? patientInfo.getDobStr() : AvsProps.DEMO_PT_DOB);
    basicDemographics.setDfn(patientInfo.getDfn());
    basicDemographics.setName(!AvsProps.IS_DEMO ? patientInfo.getName() : AvsProps.DEMO_PT_NAME);
    basicDemographics.setSex(patientInfo.getSex());
    basicDemographics.setSsn(patientInfo.getSsn());
    basicDemographics.setAge(!AvsProps.IS_DEMO ? patientInfo.getAge() : AvsProps.DEMO_PT_AGE);
    ...
    MEM_CACHE.put(encounterInfo.getFacilityNo(), encounterInfo.getPatientDfn(), "demographics", basicDemographics);
  ...
  private List<OrderInfo> getRawOrders(ISecurityContext securityContext, EncounterInfo encounterInfo) {
	  ...
	  //System.out.println("Inside SheetServiceImpl getRawOrders:");
    ...
    FacilityPrefs facilityPrefs = this.settingsService.getFacilityPrefs(securityContext, securityContext.getDivision()).getPayload();

    List<OrderInfo> result = new ArrayList<OrderInfo>();
    
/*    if( encounterInfo.getEncounterCache().getEncounters() != null)
    {
    	System.out.println("Size of encounterInfo.getEncounterCache().getEncounters() = " 
                            + encounterInfo.getEncounterCache().getEncounters().size());
    }*/
...
/*    int index =0;
    ...
    index++;
      ...
      //System.out.println("Index = " + index + " dtStr  = " + dtStr);
      
      StringBuffer tmStr = new StringBuffer("0." + StringUtils.piece(String.valueOf(encounter.getEncounterDatetime()), '.', 2));
      
      //System.out.println("Index = " + index + "tmStr  = " + tmStr);
...
/*      if( ordersInfoList == null)
      {
    	  System.out.println("ordersInfoList is null");
      }else
      {
    	  System.out.println("ordersInfoList.fmDatetime = " + ordersInfoList.getFmDatetime());
    	  
      }*/
      
  
      /* *********************************** BEGIN DEMO/TEST ****s************************************************/
/*      if (AvsProps.IS_DEMO && encounterInfo.getFacilityNo().equals(AvsProps.DEMO_STATION_NO) && 
          encounterInfo.getPatientDfn().equals(AvsProps.DEMO_PT_DFN)) {
        if (ordersInfoList == null) {
      ...
      if (orders == null) {
        ...
        for (OrderInfo order : orders) {
          if ((order.getProviderDuz() != null) &&
              ...
              boolean exist = false;
              double orderdt = order.getFmOrderDatetime();
              for (OrderInfo resultOrder : result) {
                     if (resultOrder.getFmOrderDatetime() == orderdt) {
                           exist = true;
                           break;
                     }
              }
              if (!exist) result.add(order);
          }
        }
      }
    }
  ...
  private String getMedOrderDetails(ISecurityContext securityContext, String orderNum, EncounterInfo encounterInfo) {
    ...
    ISecurityContext ddrSecCtx = getServiceSecurityContext(securityContext, encounterInfo);
    
    // first check pending outpatient orders file (#52.41) to see if med order is there
    DdrParams ddrParams = new DdrParams();
    ddrParams.setFile("52.41");
    ddrParams.setFields(".01;2;2E;13;22");
    ddrParams.setFlags("IP");
    ddrParams.setMax(10);
    ddrParams.setFrom(AvsUtils.adjustForNumericSearch(orderNum));
    ddrParams.setPart(orderNum);
    ddrParams.setXref("B");
    ...
    if (list.size() > 0) {
      ...
      if (csr == null) {
        ddrParams = new DdrParams();
        ddrParams.setFile("52");
        ddrParams.setFields(".01;39.3;100;100E;27;9;12;31;39.4;39.5");
        ddrParams.setFlags("IP");
        ddrParams.setMax(10);
        ddrParams.setFrom(AvsUtils.adjustForNumericSearch(orderNum));
        ddrParams.setPart(orderNum);
        ddrParams.setXref("APL");
        ddrParams.setUseScLister(true);
        csr = ddrRPCs.execDdrLister(ddrSecCtx, ddrParams);
  ...
  public CollectionServiceResponse<LinkedHashMap<Double, LinkedHashMap<Double, Appointment>>> 
    ...
    if (csr == null) {
      ISecurityContext ddrSecCtx = getServiceSecurityContext(securityContext, encounterInfo);
      DdrParams ddrParams = new DdrParams();
      ddrParams.setFile("403.5");
      ddrParams.setFields(".01;3E;4E;4.5E;5");
      ddrParams.setFlags("IP");
      ddrParams.setFrom(AvsUtils.adjustForNumericSearch(encounterInfo.getPatientDfn()));
      ddrParams.setPart(encounterInfo.getPatientDfn());
      ddrParams.setXref("B");
      ddrParams.setUseScLister(true);      
      csr = ddrRPCs.execDdrLister(ddrSecCtx, ddrParams);
...
/*    // check for cached remote appointments
    List<Appointment> appointments =   
         (List<Appointment>)MEM_CACHE.get(encounterInfo, "remoteAppointments");
    ...
    response.setCollection(appointments);
    MEM_CACHE.put(encounterInfo, "remoteAppointments", appointments);
  ...
  public ServiceResponse<AllergiesReactants> getAllergies(ISecurityContext securityContext, EncounterInfo encounterInfo) {
    ...
    ISecurityContext ddrSecCtx = getServiceSecurityContext(securityContext, encounterInfo);
    DdrParams ddrParams = new DdrParams();
    ddrParams.setFile("120.8");
    ddrParams.setFields(".02;3.1;20;.01");
    ddrParams.setFlags("IP");
    ddrParams.setFrom(AvsUtils.adjustForNumericSearch(encounterInfo.getPatientDfn()));
    ddrParams.setPart(encounterInfo.getPatientDfn());
    ddrParams.setXref("B");
  ...
  public CollectionServiceResponse<AllergiesReactants> getRemoteAllergies(ISecurityContext securityContext, 
      EncounterInfo encounterInfo) {
    
    CollectionServiceResponse<AllergiesReactants> response = new CollectionServiceResponse<AllergiesReactants>();
    /*List<AllergiesReactants> arList = (List<AllergiesReactants>)MEM_CACHE.get(encounterInfo, "remoteAllergies");  
    if (arList == null) {
      arList = new ArrayList<AllergiesReactants>();
      MEM_CACHE.put(encounterInfo, "remoteAllergies", arList);
                ...
                if (vhaSite != null) {
                  ...
                  try {
                    ...
                    if (allergiesReactionRpc != null) {
                      ...
                      allergiesReactants.setNoAllergyAssessment(allergiesReactions.getNoAllergyAssessment());
                      allergiesReactants.setNoKnownAllergies(allergiesReactions.getNoKnownAllergies());
                      AllergyReaction[] allergiesReactantsArray = allergiesReactions.getAllergiesReactions();
                      List<AllergyReactant> list = new ArrayList<AllergyReactant>();
                      for (int i = 0; i < allergiesReactantsArray.length; i++) {
                        ...
                        list.add(allergyReactant);
                      }
                      allergiesReactants.setAllergiesReactants(list);
                    }
...
*/    return response;
  ...
  public CollectionServiceResponse<Medication> getMedications(ISecurityContext securityContext, EncounterInfo encounterInfo) {
    ...
    DdrParams ddrParams = new DdrParams();
    ddrParams.setFile("52.41");
    ddrParams.setFields(".01;1;2;2E;6;11E");
    ddrParams.setFlags("IP");
    ddrParams.setMax(10);
    ddrParams.setFrom(AvsUtils.adjustForNumericSearch(encounterInfo.getPatientDfn()));
    ddrParams.setPart(encounterInfo.getPatientDfn());
        ...
        if (encounterInfo.getPatientDfn().equals(dfn)) {
          ...
          if (medsMap.containsKey(placerOrderNum)) {
            ...
            if (orderType.equalsIgnoreCase("NW")) {
              ...
              ddrParams = new DdrParams();
              ddrParams.setFile("52.4124");
              ddrParams.setIens("," + ien + ",");
              ddrParams.setFields(".01");
              ddrParams.setFlags("IP");
              ddrParams.setMax(10);
              ddrParams.setXref("#");
              ddrParams.setUseScLister(true);
              CollectionServiceResponse<String> csr2 = 
                  ddrRPCs.execDdrLister(securityContext, ddrParams);
              //"52.4124", "," + ien + ",", ".01", "IP", 10, 
  ...
  public CollectionServiceResponse<MedicationRdv> getRemoteOutpatientMedicationsRdv(
    ...
    CollectionServiceResponse<MedicationRdv> csr = new CollectionServiceResponse<MedicationRdv>();
/*    List<MedicationRdv> medsList = (List<MedicationRdv>)MEM_CACHE.get(encounterInfo, "remoteVaMedications");  
    if (medsList != null) {
      csr.setCollection(medsList);
    } else {
      medsList = new ArrayList<MedicationRdv>();
      csr.setCollection(medsList);
      MEM_CACHE.put(encounterInfo, "remoteVaMedications", medsList);
  ...
  public CollectionServiceResponse<Procedure> getProcedures(ISecurityContext securityContext, EncounterInfo encounterInfo) {
    ...
    log.debug("procedures: " + response.getPayload());
...
/*    if (rec != null) {
  ...
  private static RpcBroker getRpcBroker() throws Exception {
    ...
    String accessCode = StringUtils.piece(AvsProps.VISTA_AV, ';', 1);
    String verifyCode = StringUtils.piece(AvsProps.VISTA_AV, ';', 2);
    
    // Create a rpc broker instance
    RpcBroker rpcBroker = new RpcBroker(AvsProps.VISTA_SERVER, AvsProps.VISTA_PORT);
    // Signon if connected 
    if (rpcBroker.isConnected()) {
      rpcBroker.doSignon(accessCode, verifyCode);
      return rpcBroker;
    } else {
      log.error("Could not sign on to local RPC Broker server: " + AvsProps.VISTA_SERVER + ":" + AvsProps.VISTA_PORT);
    }
    return null;
  }
...
/*    Precondition.assertNotBlank("patientDfn", encounterInfo.getPatientDfn());
    ...
    CollectionServiceResponse<MedicationRdv> csr = new CollectionServiceResponse<MedicationRdv>();
/*    List<MedicationRdv> medsList = (List<MedicationRdv>)MEM_CACHE.get(encounterInfo, "remoteNonVaMedications");  
    if (medsList != null) {
      csr.setCollection(medsList);
    } else {
      medsList = new ArrayList<MedicationRdv>();
      csr.setCollection(medsList);
      MEM_CACHE.put(encounterInfo, "remoteNonVaMedications", medsList);
  ...
  public CollectionServiceResponse<PceData> getPceData(ISecurityContext securityContext, EncounterInfo encounterInfo) {
    ...
    List<Encounter> enc = encounterInfo.getEncounterCache().getEncounters();
    for (Encounter encounter : enc) {
      encounterLocationIen = encounter.getLocationIen();
      ...
      pceData.setClinicIen(encounterLocationIen);
      pceData.setClinicName(encounter.getLocationName());

      //System.out.println("PCE RESULTS");
      //System.out.println("--------------");
        ...
        for (String s : pceResults) {
          ...
          if ((s.substring(0, 3).equals("PRV") && s.charAt(3) != '-')) {
            ...
            if (!providerDuz.isEmpty()) {
              ...
              if (title != null) {
                ...
                try {
                  ...
                  String rtn = miscRpcs.getVariableValue(securityContext, arg).getPayload();
                  titleIen = StringUtils.piece(rtn, 9);
                  if (!titleIen.isEmpty()) {
                    arg = "@\"^DIC(3.1," + titleIen + ",0)\"";
                    title = miscRpcs.getVariableValue(securityContext, arg).getPayload();
                    if (title != null) {
            ...
            if ((instIen == null) || (instName == null)) {
              DdrParams ddrParams = new DdrParams();
              ddrParams.setFile("44");
              ddrParams.setFields(".01;1;3;3E");
              ddrParams.setFlags("IP");
              ddrParams.setXref("#");
              ddrParams.setScreen("I + Y=" + clinicIen);
              ddrParams.setUseScLister(true);
              CollectionServiceResponse<String> csr = ddrRPCs.execDdrLister(ddrSecCtx, ddrParams);
              List<String> list = (List<String>)csr.getCollection();
              if (list.size() > 0) {
...
/*                ddrParams = new DdrParams();
                ...
                ddrParams.setFields(".07;1");
                ddrParams.setFlags("IP");
                ddrParams.setXref("AD");
                ddrParams.setScreen("I $P($G(^(0)),U,7)=\"" + instNo + "\"");
                ddrParams.setUseScLister(true);
                csr = ddrRPCs.execDdrLister(ddrSecCtx, ddrParams);*/
                ...
                list = patientInfoDao.getPceData(instNo);
                
                //list = (List<String>)csr.getCollection();
                
/*          	  if( list != null)
        	  {
        		  System.out.println("The size of results list = " + list.size());
        		  
        		  for( String eachResult : list)
        		  {
                ...
                if (list.size() > 0) {
                  ...
                  //System.out.println("instIen = " + instIen);
                }
              }
        ...
        if (title != null) {
          ...
          try {
            ...
            if (!titleIen.isEmpty()) {
              arg = "@\"^DIC(3.1," + titleIen + ",0)\"";
              rtn = miscRpcs.getVariableValue(securityContext, arg).getPayload();
              if (rtn != null) {
...
/*  public ServiceResponse<String> saveHealthFactor(ISecurityContext securityContext, 
  ...
  public CollectionServiceResponse<String> getPatientTeamMembers(ISecurityContext securityContext, 
    ...
    results = patientInfoDao.getPatientTeamMembers(encounterInfo.getPatientDfn(), primaryCareTeam);
    response.setCollection(results); 
  
    
    /*// Get primary care team for patient from patient team assignment file
    DdrParams ddrParams = new DdrParams();
    ddrParams.setFile("404.42");
    ddrParams.setFields(".02;.03;.03E;.01");
    ddrParams.setFlags("IP");
    ddrParams.setFrom(AvsUtils.adjustForNumericSearch(encounterInfo.getPatientDfn()));
    ddrParams.setPart(encounterInfo.getPatientDfn());
    ddrParams.setXref("B");
    ...
    if (teamIen != null) {
      ...
      ddrParams = new DdrParams();
      ddrParams.setFile("404.57");
      ddrParams.setFields(".01;.01E");
      ddrParams.setFlags("IP");
      ddrParams.setXref("C");
      ddrParams.setScreen("I $P($G(^(0)),U,2)=" + teamIen + ",+$$ACTTP^SCMCTPU(Y)=1");
      ddrParams.setUseScLister(true);
      csr = ddrRPCs.execDdrLister(ddrSecCtx, ddrParams);
      list = (List<String>)csr.getCollection();
      for (String line : list) {
        ...
        if (teamPosition.contains("CARE MANAGER") || 
          ...
          ddrParams = new DdrParams();
          ddrParams.setFile("404.52");
          ddrParams.setFields(".02;.03E");
          ddrParams.setFlags("IP");
          ddrParams.setXref("B");
          ddrParams.setScreen("I $P($G(^(0)),U,1)=" + teamPositionIen);
          ddrParams.setUseScLister(true);
          csr = ddrRPCs.execDdrLister(ddrSecCtx, ddrParams);
          list = (List<String>)csr.getCollection();
          lastDt = 0.0;
...
/*    if( results != null)
    {
        ...
        {
        	System.out.println("Result = " + eachResult);
        }
    }*/
  ...
  public ServiceResponse<EncounterCache> getEncounterCacheById(Long id) {
    EncounterCache encounterCache = encounterCacheDao.findById(id);
    ServiceResponse<EncounterCache> sr = new ServiceResponse<EncounterCache>();
    sr.setPayload(encounterCache);
    return sr;
  }  
  
  public ServiceResponse<EncounterCache> getEncounterCache(String facilityNo, String patientDfn, 
		  List<String> locationIens, List<Double> datetimes, String docType) {

	  String key = getEncounterCacheKey(facilityNo, patientDfn, locationIens, datetimes, docType);

	  EncounterCache encounterCache = (EncounterCache)MEM_CACHE.get(key, "encounterCache");

	  //if (encounterCache == null) {    
	  encounterCache = encounterCacheDao.find(facilityNo, patientDfn, locationIens, datetimes, docType);
	  //}

	  ServiceResponse<EncounterCache> sr = null;
	  if (encounterCache != null) {
		  MEM_CACHE.put(key, "encounterCache", encounterCache);
		  sr = new ServiceResponse<EncounterCache>();
		  sr.setPayload(encounterCache);
	  }
	  return sr;    
  }
  ...
  public CollectionServiceResponse<EncounterCache> getEncounterCacheByDates(String facilityNo, Date startDate, Date endDate) {
    CollectionServiceResponse<EncounterCache> sr = new CollectionServiceResponse<EncounterCache>();
    List<EncounterCache> list = encounterCacheDao.findByDates(facilityNo, startDate, endDate);
    sr.setCollection(list);
    return sr;     
  }
  
  public ServiceResponse<EncounterCache> saveEncounterCache(EncounterCache encounterCache) {
	  encounterCache = encounterCacheDao.save(encounterCache);

	  if ((encounterCache != null) && (encounterCache.getEncounters() != null)) {
		  for (Encounter encounter : encounterCache.getEncounters()) {
			  encounter.setEncounterCacheId(encounterCache.getId());
			  encounter = encounterDao.update(encounter);

			  if (encounter.getProviders() != null) {
				  for (EncounterProvider encounterProvider : encounter.getProviders()) {
					  encounterProvider.setEncounterId(encounter.getId());
					  encounterProviderDao.update(encounterProvider);
				  }
			  }
		  }
	  }

	  encounterCache = this.getEncounterCacheById(encounterCache.getId()).getPayload();
	  MEM_CACHE.put(getEncounterCacheKey(encounterCache), "encounterCache", encounterCache);
	  ServiceResponse<EncounterCache> sr = new ServiceResponse<EncounterCache>();
	  sr.setPayload(encounterCache);
	  return sr;
  }

  public ServiceResponse<EncounterCache> updateEncounterCache(EncounterCache encounterCache) {
    encounterCache = encounterCacheDao.update(encounterCache);
    encounterCache = this.getEncounterCacheById(encounterCache.getId()).getPayload();
    MEM_CACHE.put(getEncounterCacheKey(encounterCache), "encounterCache", encounterCache);
    ServiceResponse<EncounterCache> sr = new ServiceResponse<EncounterCache>();
    sr.setPayload(encounterCache);
  ...
  public void deleteEncounterCache(EncounterCache encounterCache) {
    encounterCacheDao.remove(encounterCache.getId());
  }  

  public ServiceResponse<EncounterProvider> saveEncounterProvider(EncounterProvider encounterProvider) {
    encounterProvider = encounterProviderDao.update(encounterProvider);
    ServiceResponse<EncounterProvider> sr = new ServiceResponse<EncounterProvider>();
    sr.setPayload(encounterProvider);
    return sr;
  }  
  
  private String getEncounterCacheKey(String facilityNo, String patientDfn, 
      List<String> locationIens, List<Double> datetimes, String docType) {
    StringBuffer sb = new StringBuffer();
    sb.append(facilityNo);
    sb.append("^");
    sb.append(patientDfn);
    sb.append("^");
    for (String loc : locationIens) {
      sb.append(loc);
      sb.append(",");
    }    
    sb.append("^");
  ...
  public String getEncounterCacheKey(EncounterCache encounterCache) {
    StringBuffer sb = new StringBuffer();
    sb.append(encounterCache.getFacilityNo());
    sb.append("^");
    sb.append(encounterCache.getPatientDfn());
    sb.append("^");    
    for (Encounter encounter : encounterCache.getEncounters()) {
      sb.append(encounter.getLocationIen());
      sb.append(",");
    }
    sb.append("^");   
    for (Encounter encounter : encounterCache.getEncounters()) {
      sb.append(encounter.getEncounterDatetime());
      sb.append(",");
    }    
    sb.append("^");
    sb.append(encounterCache.getDocType());
    return sb.toString();    
  }
  ...
  public CollectionServiceResponse<MedDescription> getMedicationDescriptions(List<String> medNdcs) {
    ...
    for (String ndc : medNdcs) {
      ...
      if (ndc != null) {
        medDescription = (MedDescription)MEM_CACHE.get(ndc, "medDescription");
        if (medDescription == null) {
          medDescription = medDescriptionsDao.findByNdc(ndc);
        }
        if (medDescription != null) {
          MEM_CACHE.put(ndc, "medDescription", medDescription);
    ...
    if (!ndcsNotCached.isEmpty()) {
      ...
      if (dbConnectionPool == null) {
        try {
          if (AvsProps.MIL_ENABLED) {
            dbConnectionPool = new DBConnectionPool(AvsProps.MIL_DRIVER_TYPE, AvsProps.MIL_DB_TYPE,
                AvsProps.MIL_DB_SERVER, Integer.valueOf(AvsProps.MIL_DB_PORT).intValue(),  
                AvsProps.MIL_DB_NAME, AvsProps.MIL_DOMAIN, AvsProps.MIL_USERNAME, 
                AvsProps.MIL_PASSWORD, AvsProps.MIL_DRIVER, 
  ...
  public CollectionServiceResponse<HealthFactor> getPatientHealthFactors(ISecurityContext securityContext, EncounterInfo encounterInfo) {
    ...
    ddrSecCtx.setSocketTimeout(60000);
    DdrParams ddrParams = new DdrParams();
    ddrParams.setFile("9000010.23");
    ddrParams.setFields(".01;.01E;.02;.02E;.03;.03E;.04;80102;81101");
    ddrParams.setFlags("IP");
    ddrParams.setFrom(AvsUtils.adjustForNumericSearch(encounterInfo.getPatientDfn()));
    ddrParams.setPart(encounterInfo.getPatientDfn());
    ddrParams.setXref("C");
    ddrParams.setScreen("I $P(^(0),U,2)'=\"\"");
    ddrParams.setUseScLister(true);
    CollectionServiceResponse<String> csr = ddrRPCs.execDdrLister(ddrSecCtx, ddrParams);
  ...
  public ISecurityContext getServiceSecurityContext(ISecurityContext securityContext, EncounterInfo encounterInfo) {
    
    if (!DDR_SEC_CTX_MAP.containsKey(encounterInfo.getFacilityNo())) {
      ISecurityContext secCtx = null;
      String serviceDuz = this.settingsService.getServiceDuz(securityContext, encounterInfo.getFacilityNo()).getPayload();
      String facilityNo = this.settingsService.getFacilityNoForDivision(securityContext, encounterInfo.getFacilityNo()).getPayload();
      ...
      if( facNum == null && facilityNo != null)
      {
    	  facNum = facilityNo.trim();
      }
      else
      {
    	  facNum = facNum.trim();

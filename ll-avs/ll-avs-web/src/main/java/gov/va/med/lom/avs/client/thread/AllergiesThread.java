...
public class AllergiesThread extends SheetDataThread {
  ...
  public void run() {
    ...
    try {
      ServiceResponse<AllergiesReactants> sr = super.getSheetService()
          .getAllergies(super.securityContext, super.getEncounterInfo());
      AllergiesReactants localAllergiesReactions = sr.getPayload();
      
      CollectionServiceResponse<AllergiesReactants> csr = 
          super.getSheetService().getRemoteAllergies(super.securityContext, super.getEncounterInfo());
      List<AllergiesReactants> remoteAllergiesReactions = (List<AllergiesReactants>)csr.getCollection();
      
      List<AllergyReactant> localAllergies = localAllergiesReactions.getAllergiesReactants();
      List<AllergyReactant> remoteAllergies = new ArrayList<AllergyReactant>();
      boolean remoteNoAllergyAssessment = true;
      boolean remoteNoKnownAllergies = true;
      for (AllergiesReactants ar : remoteAllergiesReactions) {
        remoteAllergies.addAll(ar.getAllergiesReactants());
        remoteNoAllergyAssessment = remoteNoAllergyAssessment && ar.getNoAllergyAssessment();
        remoteNoKnownAllergies = remoteNoKnownAllergies && ar.getNoKnownAllergies();
      }
      
      if ((localAllergiesReactions.getNoAllergyAssessment() == true) &&
          ((remoteAllergies == null) || (remoteAllergies.size() == 0) || remoteNoAllergyAssessment)) {
        ...
        allergiesReactionsJson.setNoAllergyAssessment(true);
                 ...
                 ((remoteAllergies == null) ||  (remoteAllergies.size() == 0) || remoteNoKnownAllergies)) {
        ...
        while (en.hasMoreElements()) {
          ...
          if (reactions.length > 0) {
            if (!reactions[0].trim().isEmpty()) {
              description.append(" (");
              description.append(AvsWebUtils.adjustCapitalization(AvsUtils.delimitString(reactions, true)));
              description.append(")");
            }
          }
          /*

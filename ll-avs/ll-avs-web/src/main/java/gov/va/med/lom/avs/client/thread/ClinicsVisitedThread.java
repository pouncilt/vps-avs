...
public class ClinicsVisitedThread extends SheetDataThread implements QSCallBack {
  ...
  public void run() {
	  ...
	  //System.out.println("Inside ClinicsVisitedThread run method");
    ...
    try {
      ...
      for (Encounter encounter : sortedEncounters) {
    	 // System.out.println("encounter.getLocationName() = " + encounter.getLocationName());
        clinicNames.add(encounter.getLocationName());
      }      
      List<String> clinicNameTranslations = null;
      try {
        CollectionServiceResponse<String> translationsResponse = super.getSettingsService()
          .translateStrings(super.securityContext, super.facilityNo, super.getLanguage(), TranslationTypeEnum.LOCATION_NAME, clinicNames);
...
/*        if(clinic != null)
        ...
        for (EncounterProvider provider : encounter.getProviders()) {
          if (provider.getProviderName() != null && !provider.getProviderName().isEmpty()) {
            ...
            //System.out.println("provider.getProviderName() = " + provider.getProviderName());
       ...
       // System.out.println("clinicVisited.getClinic() = " + clinicVisited.getClinic());
        
        clinicsVisited.add(clinicVisited);
        items.add(fullDescription.toString().trim());
        
        //System.out.println("fullDescription.toString().trim() = " + fullDescription.toString().trim());
      }

...
public class InfoSheetsAction extends BaseCardAction {
  ...
  public void prepare() throws Exception {
    ...
    for (String s : dts) {
      ...
      // required params
      encounter.setLocationIen(locations[i]);
      encounter.setLocationName("");
    ...
    EncounterCache ec = new EncounterCache();
    ec.setPatientDfn(super.patientDfn);
    ec.setEncounters(encounters);
    encounterInfo.setEncounterCache(ec);
    ...
    String xmlResponse = sendRequest(AvsProps.KRAMES_BASE_URL + AvsProps.KRAMES_SEARCH_PATH, xmlRequest);
	...
	public String kramesGetContent() {
	  ...
	  for (int i = 0; i < contentIdArr.length; i++) {
	    ...
	    temp.append(contentObj.getHtml());
	    temp.append("<br/>");
	  }
    ...
    String title = null;
	  ...
	  if (media == MEDIA_HTML) {
	    ...
	    html.append(title);
	    html.append("</title>");
	    html.append("<link rel=\"stylesheet\" type=\"text/css\" href=\"" + baseURI + AvsProps.KRAMES_LOCAL_CSS + "\" />");
	    ...
	    try {
          ...
          HttpGet request = new HttpGet(baseURI + AvsProps.KRAMES_LOCAL_CSS);
        ...
        iTextRenderer.setDocument(doc, baseURI + AvsProps.KRAMES_LOCAL_IMG);
	...
	private ContentObj doGetDocumentContent(String contentTypeId, String contentId) {
	  ...
	  String xmlRequest = sendRequest(AvsProps.KRAMES_BASE_URL + AvsProps.KRAMES_CONTENT_PATH, xml);
      ...
      while (fromIndex >= 0) {
        ...
        if (fromIndex >= 0) {
          ...
          if (st < sb.length()) {
            ...
            try {
              String imagePath = AvsProps.KRAMES_LOCAL_IMG + imageId + ".jpg";
              File localImageFile = new File(AvsProps.KRAMES_LOCAL_WEB + imagePath);
              if (!localImageFile.exists()) {
                URL url = new URL(AvsProps.KRAMES_BASE_URL + "/" + imageId + ".img");

...
LLVA.AVS.Wizard = {
    ...
    params: {
        ...
        urls: {
            ...
            setFontClass:           '/w/s/avs/setFontClass.action',
            setLocked:              '/w/s/avs/setLocked.action',
            setPrintClinSvcDesc:    '/w/s/avs/setPrintClinicalServiceDescriptions.action',
    ...
    httpArgsAreValid: function() {
        ...
        var isLocal = this.httpArgs.localhost && (Boolean(this.httpArgs.localhost) == true);
        if (this.httpArgs.servers && !isLocal) {
            var servers = this.httpArgs.servers.split(',');
            this.setParam('servers', servers);            
            this.getUserDivision();
        } else if (!this.httpArgs.servers && !isLocal) {
            this.getServers();
        } else {            
            var obj = [this.getParam('hostname')];                
            this.setParam('servers', obj);
            this.getUserDivision();
        }         
        return true;
    },

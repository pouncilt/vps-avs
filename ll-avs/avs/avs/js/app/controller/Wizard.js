Ext.define('LLVA.AVS.Wizard.controller.Wizard', {
    ...
    setAvsButtonsEnabled: function(enabled) {
        Ext.getCmp('refreshBtn').setDisabled(!enabled)
        Ext.getCmp('commentsBtn').setDisabled(!enabled);
        Ext.getCmp('optionsBtn').setDisabled(!enabled);
        Ext.getCmp('editContentBtn').setDisabled(!enabled);
        Ext.getCmp('infoSheetsBtn').setDisabled(!enabled);                    
        Ext.getCmp('printBtn').setDisabled(!enabled);  
        Ext.getCmp('pdfBtn').setDisabled(!enabled);         
    },    
        ...
        Ext.Ajax.request({
            ...
            success: function (result, request) { 
                ...
                if (allEncounters.length == 0) {
                    this.setAvsButtonsEnabled(false);                    
                ...
                menu: [{
                    ...
                    menu: [{
                        ...
                        handler: function(item) {
                            ...
                            this.setPrintClinicalServiceDescriptions(this.printAllServiceDescriptions,
                                                                     this.selectedServiceDescriptions);
                        }
                    },{
                        xtype: 'menucheckitem',
        ...
        Ext.Ajax.request({
            ...
            success: function(response) {
                ...
                if (response) {
                    ...
                    APP_GLOBAL.getController('Wizard').fontSize(this.curFontClass, false);
                    ...
                    this.setAvsButtonsEnabled(true);
    ...
    insertComments: function(content) {
        ...
        // clean up Krames content for better formatting
        comments = this.replaceAll(comments, '\t', '');
        comments = this.replaceAll(comments, '<p>', '');
        comments = this.replaceAll(comments, '<\/p>', '');
        comments = this.replaceAll(comments, '\n', ' ');
        Ext.DomQuery.selectNode('#comments-div').innerHTML = 
            '<pre id="comments-area">' + comments + '</pre>';   
    ...
    saveContentEdits: function(button) {
        ...
        var win = this.notificationWindow('Updating AVS', 'Saving edits.');   
    ...
    saveLockedStatus: function(isLocked) {
        Ext.Ajax.request({
            ...
            failure: function(response, requestOptions) {
                LLVA.AVS.Wizard.handleAjaxError(response.status, 'saveLockedStatus', this.saveLockedStatus);
            }            
        });     
    },

    setPrintClinicalServiceDescriptions: function(printAll, selected) {
        Ext.Ajax.request({
            url: LLVA.AVS.Wizard.getUrl('setPrintClinSvcDesc'),
            params: {
                patientDfn: LLVA.AVS.Wizard.getParam('patientDfn'),
                datetimes: LLVA.AVS.Wizard.getParam('datetimes'),
                locationIens: LLVA.AVS.Wizard.getParam('locationIens'),
                locationNames: LLVA.AVS.Wizard.getParam('locationNames'),
                printAllServiceDescriptions: printAll,
                selectedServiceDescriptions: selected
            },
            scope: this,
            success: function(response) {
            },
            failure: function(response, requestOptions) {
                LLVA.AVS.Wizard.handleAjaxError(response.status, 'setPrintClinicalServiceDescriptions', 
                    this.setPrintClinicalServiceDescriptions);
            }   
        });     
    },    
        ...
        for (i = 0; i < 2; i++) {
            ...
            for (j = 0; j < remoteMeds.length; j++) {            
                ...
                if (med.type == 'VA') {                
                    ...
                    this.remoteVaMedicationsHtml += "<div class=\"med-detail\">" + this.refillsTitle + ": " + med.refillsRemaining;
        ...
        this.exportCharts(function(chartFilenames) {              
            ...
            var pdfWin = new Ext.Window({
                ...
                buttons: [{                
                    ...
                    icon: 'images/printer-plus.png', 
                    handler: function() {
					alert('Inside pdf: function pdfObject = ' + pdfObject);
                ...
                listeners: {
                    ...
                    'afterrender': function() { 
                        ...
                        try { 
							...
							alert('Inside pdf: function after new pdfObject = ' + pdfObject);
    ...
    fontSize: function(cls, saveToServer) {        
        ...
        Ext.Ajax.request({
            url: LLVA.AVS.Wizard.getUrl('setFontClass'),
            params: {
                patientDfn: LLVA.AVS.Wizard.getParam('patientDfn'),
                datetimes: LLVA.AVS.Wizard.getParam('datetimes'),
                locationIens: LLVA.AVS.Wizard.getParam('locationIens'),
            ...
            failure: function(response, requestOptions) {
                LLVA.AVS.Wizard.handleAjaxError(response.status, 'fontSize', 
                    this.fontSize);
            }   
        });          
    ...
    print: function(action, url) {                     
		...
		alert('Inside print: fucntion url = ' + url);
        ...
        var callback = function(chartFilenames) {
            this.chartFilenames = chartFilenames;
			alert('Inside print: fucntion chartFilenames = ' + this.chartFilenames);
    ...
    notificationWindow : function(title, msg) {
        var win = new Ext.Window({
            id: 'win',
            title: title,
            html:'<p style="margin-top: 10px;font: 13px Tahoma,Verdana,Helvetica,sans-serif;text-align: center;">' + msg + '</p>',

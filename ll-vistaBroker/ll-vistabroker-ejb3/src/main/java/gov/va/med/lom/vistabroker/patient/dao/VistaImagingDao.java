...
public class VistaImagingDao extends BaseDao {
    private static final Log log = LogFactory.getLog(VistaImagingDao.class);
  ...
  /*
    ...
    1. use RPC call TIU CREATE RECORD and set back IEN of created note
    2. use RPC call MAG4 ADD IMAGE with patient information and image
       information.  set back a file path and file name where
       image should be saved to.  Also set back IEN in IMAGE file
  ...
  */
  public List<String> importVistaImage(List<String> nodeList) throws VistaBrokerException {
    ...
    public List<VistaImageQueueInfo> retrieveVistaImages(List<String> params) throws VistaBrokerException {
        setDefaultContext("VPS AVS INTERFACE");
        setDefaultRpcName("VPS EXPORT AVS IMAGES");

        List<VistaImageQueueInfo> vistaImageQueueInfoList = new ArrayList<VistaImageQueueInfo>();
        HashMap<String, Object> textHashMap = new HashMap<String, Object>();
        for(int i = 0; i < params.size(); i++) {
            textHashMap.put(Integer.toString(i+1), params.get(i));
        }

        List<String> results = lCall(textHashMap);

        for(String x: results){
            System.out.println("x: " + x);
            vistaImageQueueInfoList.add( new VistaImageQueueInfo(StringUtils.piece(x, 1), StringUtils.piece(x, 2), StringUtils.piece(x, 3)));
        }

        return vistaImageQueueInfoList;
    }

    public List<VistaImageInfo> retrieveVistaImageDescriptions(List<Object> params) throws VistaBrokerException {
        setDefaultContext("VPS AVS INTERFACE");
        setDefaultRpcName("VPS AVS IMAGE LIST");

        List<VistaImageInfo> vistaImageInfoList = new ArrayList<VistaImageInfo>();
        List<String> results = lCall(params);
        boolean readResultHeader = true;
        int sequenceNumber = 0;

        for(String x: results){
            Date importDate;
            VistaImageInfo vistaImageInfo = new VistaImageInfo();
            vistaImageInfo.setPatientDFN((String) params.get(0));

            System.out.println("x: " + x);
            if(readResultHeader) {
                sequenceNumber = Integer.valueOf(StringUtils.piece(x, 1));
                if (sequenceNumber == 1){
                    vistaImageInfo.setSiteCode(Integer.toString(results.size() - 1));
                    readResultHeader = false;
                    continue;
                } else if (sequenceNumber == 0){
                    vistaImageInfo.setSiteCode(StringUtils.piece(x, 2));
                    vistaImageInfoList.add(vistaImageInfo);
                    break;
                }
            }

            importDate = FMDateUtils.fmDateTimeToDate(StringUtils.piece(x, 1));
            vistaImageInfo.setImageSavedDateTimeStamp(importDate);
            vistaImageInfo.setDocumentDate(importDate);
            vistaImageInfo.setCapturedDate(importDate);
            vistaImageInfo.setExamDateTimeStamp(importDate);
            vistaImageInfo.setDisplayDate(importDate);
            vistaImageInfo.setSequenceNumber(sequenceNumber);
            vistaImageInfo.setImageIen(StringUtils.piece(x, 2));
            vistaImageInfo.setFilename(StringUtils.piece(x, 3));
            vistaImageInfo.setCapturedBy(StringUtils.piece(x, 4));
            vistaImageInfoList.add(vistaImageInfo);
        }

        return vistaImageInfoList;
    }
}
